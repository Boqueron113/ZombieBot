...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .header-left .sub{font-family:'Share Tech Mono',monospace;font-size:.7rem;color:var(--rojo)}
  ...
  .item-card .nombre{font-family:'Bebas Neue',sans-serif;font-size:.9rem}
  ...
  .btn-accion{background:var(--rojo);color:#fff;border:none;padding:.3rem .7rem;border-radius:5px;font-family:'Bebas Neue',sans-serif;font-size:.8rem;cursor:pointer;transition:background .15s}
  .btn-accion:hover{background:#96281b}
  .btn-accion:disabled{background:#444;cursor:not-allowed;color:#666}
  .btn-verde{background:var(--verde)!important}
  .btn-verde:hover{background:#1e8449!important}
  ...
  .empty-msg{color:var(--texto-dim);font-family:'Share Tech Mono',monospace;font-size:.8rem;padding:2rem;text-align:center;grid-column:1/-1}
...
<div class="header">
  <div class="header-left">
    <h1>🏪 Tienda del Refugio</h1>
    <div class="sub">{{ nombre }} • TOKEN VÁLIDO 15 MIN</div>
  </div>
  <div class="header-right">
    <div class="tapas" id="tapas-display">💰 {{ tapas }} tapas</div>
    {% if puede_comprar %}
    <div class="zona-ok">✅ Refugio — compra y vende disponibles</div>
    {% else %}
    <div class="zona-no">❌ Ve a !explorar refugio para comprar/vender</div>
    {% endif %}
  </div>
</div>
<div class="tabs">
  <button class="tab-btn active" onclick="showTab('comprar',this)">🛒 COMPRAR</button>
  <button class="tab-btn" onclick="showTab('vender',this)">💸 VENDER</button>
</div>
<div id="tab-comprar">
  <div class="toolbar">
    ...
    <div class="items-grid cat-grid" data-cat="{{ loop.index }}">
      ...
      <div class="item-card" data-tipo="{{ item.tipo }}" data-nombre="{{ item.nombre|lower }}" data-id="{{ item.id }}">
        ...
        <div class="precio-row">
          ...
          {% if puede_comprar %}
          <button class="btn-accion" onclick="comprar('{{ item.id }}','{{ item.nombre }}',{{ item.precio_compra }})">COMPRAR</button>
          {% else %}
          <button class="btn-accion" disabled>NO DISPONIBLE</button>
          {% endif %}
        </div>
      </div>
      {% endfor %}
...
<div id="tab-vender" class="hidden">
  <div class="content" style="margin-top:1rem">
    <div class="cat-title">💸 INVENTARIO — Items que puedes vender</div>
    <div id="inv-grid" class="items-grid"><div class="empty-msg">Cargando...</div></div>
  </div>
</div>
<div id="toast"></div>
<div class="footer">APOCALIPSIS ZOMBIE • !tienda en Discord para renovar el enlace</div>
<script>
const TOKEN = "{{ token }}";
...
function showTab(tab, btn) {
  ...
  if (tab === 'vender') cargarInventario();
}
async function comprar(itemId, nombre, precio) {
  if (tapasActuales < precio) { showToast('No tienes suficientes tapas', true); return; }
  const btn = event.target; btn.disabled = true; btn.textContent = '...';
  try {
    const r = await fetch('/api/comprar', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:TOKEN,item_id:itemId,cantidad:1})});
    const d = await r.json();
    if (d.ok) { tapasActuales = d.tapas_restantes; document.getElementById('tapas-display').textContent = '💰 ' + tapasActuales + ' tapas'; showToast('✅ ' + d.mensaje); }
...
async function cargarInventario() {
  ...
  grid.innerHTML = '<div class="empty-msg">Cargando inventario...</div>';
  try {
    const r = await fetch('/api/inventario/' + TOKEN);
    const d = await r.json();
    if (!d.ok) { grid.innerHTML = '<div class="empty-msg">' + d.error + '</div>'; return; }
    const vendibles = Object.entries(d.inventario).filter(([id,cant]) => !id.endsWith('_dur') && cant > 0 && window.ITEMS_INFO[id]);
...
async function vender(itemId, nombre, precio, btn) {
  ...
  try {
    ...
    const d = await r.json();
    if (d.ok) { tapasActuales = d.tapas_restantes; document.getElementById('tapas-display').textContent = '💰 ' + tapasActuales + ' tapas'; showToast('✅ ' + d.mensaje); setTimeout(cargarInventario,400); return; }
    else showToast('❌ ' + d.error, true);
  } catch(e) { showToast('❌ Error de conexión', true); }
  btn.disabled = false; btn.textContent = 'VENDER 1';
}
let filtroActivo = 'todos';
function setFiltro(f, btn) {
  filtroActivo = f;
  document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active'); filtrar();
}
function filtrar() {
  const q = document.getElementById('search').value.toLowerCase();
  document.querySelectorAll('#tab-comprar .item-card').forEach(c => {
    const mt = filtroActivo === 'todos' || c.dataset.tipo === filtroActivo;
    const mq = !q || (c.dataset.nombre||'').includes(q) || (c.dataset.id||'').includes(q);
    c.classList.toggle('hidden', !(mt && mq));
  });
